      ...
      <div class="p-5 rounded" style="background-color: rgba(255, 250, 240, 0.8);">
        <h3 class="text-center">Offer</h3>
             <div class="card bg-transparent border-0">
               <div class="row">
              <?php
              $query = "SELECT * FROM offer WHERE account_id = '".$_GET['account_id']."'";
              $result = mysqli_query($connect, $query);
              while ($row = mysqli_fetch_array($result)) { ?>
              <div class="col-lg-3 mt-5">
                <img src="../supplier/asset/images/<?php echo $row['file_name'] ?>" width="100%" height="250px">
                <h4 class="text-center"><?php echo $row['title'] ?></h4>
                <div class="text-center"><a class="btn btn-dark btn-sm rounded-0" href="offerview.php?offer_id=<?php echo $row['offer_id'] ?>">See more Details</a></div>
              </div>
            <?php } ?>
          </div>
             </div>
